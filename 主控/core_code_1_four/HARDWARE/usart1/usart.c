...
//V1.4修改说明
//1,修改串口初始化IO的bug
//2,修改了USART_RX_STA,使得串口最大接收字节数为2的14次方   WTF??? #(-_-)|||
...
float main_to_motorf[10]={0xaf,0x64,0x64,0x64,0x64,0x00,0x00,0x00,0x00,0xff};      //发送给电机的数据
u8 main_to_motoru8[10]={0xaf,0x64,0x64,0x64,0x64,0x00,0x00,0x00,0x00,0xff};
...
//由沈一祺于2022/5/18 12：46 修改

/************************************左右和前后同时控制************************************/
main_to_motorf[1]=100 + (100-PC_to_MCU[4])*((float)speed_rata)/100 + (PC_to_MCU[3]-100)*((float)speed_rata)/100;
main_to_motorf[2]=100 - (100-PC_to_MCU[4])*((float)speed_rata)/100 + (PC_to_MCU[3]-100)*((float)speed_rata)/100;

if(main_to_motorf[1] > 200) 	main_to_motorf[1] =200;
if(main_to_motorf[2] > 200) 	main_to_motorf[2] =200;
if(main_to_motorf[1] < 0) 	main_to_motorf[1] =0;
if(main_to_motorf[2] < 0) 	main_to_motorf[2] =0;
/*****************************************************************************************/
...
/*	if(PC_to_MCU[3]!=0x64)
	{
		if(PC_to_MCU[3]>0x64)
		{
			main_to_motorf[1]=100+(PC_to_MCU[3]-100)*((float)speed_rata)/100;
			main_to_motorf[2]=100+(PC_to_MCU[3]-100)*((float)speed_rata)/100;
		}
		else
		{
			main_to_motorf[1]=100-(100-PC_to_MCU[3])*((float)speed_rata)/100;
			main_to_motorf[2]=100-(100-PC_to_MCU[3])*((float)speed_rata)/100;
		}
	}
	...
	if(PC_to_MCU[3]==0x64)
	{
		...
		if(PC_to_MCU[4]!=0x64)
		{
			if(PC_to_MCU[4]>0x64)
			{
				//由沈一祺在2022/5/18 12：07 添加 用于添加对于水下机器人左右转向的线性控制
				main_to_motorf[1]=100-(100-PC_to_MCU[4])*((float)speed_rata)/100;
				main_to_motorf[2]=100+(100-PC_to_MCU[4])*((float)speed_rata)/100;
...
/****************************************************************************************/
	...
	if(PC_to_MCU[6]>0x64)
	{
		main_to_motorf[3]=100-(PC_to_MCU[6]-100)*((float)speed_rata)/100;
		main_to_motorf[4]=100-(PC_to_MCU[6]-100)*((float)speed_rata)/100;
...
if(main_to_motorf[3] > 200) 	main_to_motorf[3] =200;
if(main_to_motorf[4] > 200) 	main_to_motorf[4] =200;
if(main_to_motorf[3] < 0) 	main_to_motorf[3] =0;
if(main_to_motorf[4] < 0) 	main_to_motorf[4] =0;
...
int abs( int input); //不知道是什么鬼   fix   "warning": "function \"abs\" declared implicitly",
...
返回值：void
*/
void float_to_u8(void)                    //把float变量变成u8
{
	u8 i=0;
	for(i=0;i<10;i++)
