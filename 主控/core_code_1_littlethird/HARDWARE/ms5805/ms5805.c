 ...
 * 输出  ：无
 */
float depth_caculate(uint32_t depth)//一次只能发8个字节
{
	static int depth_base=0;
	static uint8_t init_flag = 0;
	static int depth_num = 0;
	static int deep_last=0;
	float deep=0;
	if(!init_flag)
	{
		depth_base+=(depth/10);
	...
	{
		deep=deep*0.2f+deep_last*0.8f;//初始化后deep_last一定是0
		deep_last=deep;
		deep=(float)(((float)depth-depth_base)/0.98f);//
	}
...
volatile float real_deep;
void deep_out(void)
{
	MS5837_Temperature_Begin_conversion();
	MS5837_Temperature_Read();             //设置深度传感器的转化速度
	MS5837_Pressure_Begin_Conversion();
	MS5837_Pressure_Read();
	deep_result=MS5837_Pressure_ReadAndDMP();
	real_deep=depth_caculate((int)deep_result.P)/10000.0f;//m
	//real_deep=deep_result.P/10000.0f;//m
	//由沈一祺在2022/5/16/21：04 修改 校准深度值
	//real_deep  = 11.5 * (real_deep-465.0) / (467.0 - 465.0) / 100 - 526.08;
}
